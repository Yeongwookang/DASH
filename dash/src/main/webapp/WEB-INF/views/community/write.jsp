		...
		<div class="ps-5 pe-5 mt-4">
			...
			<form name="communityForm" method="post">
				...
				<select name="categoryNum" id="categoryNum" class="form-select" style="width: 100%">
					<option value="" ${categoryNum==""?"selected='selected'":""}>유형을 선택해주세요</option>  
					<option value="1" ${categoryNum=="1"?"selected='selected'":""}>조직 문화에 대해 제안합니다.</option>
					<option value="2" ${categoryNum=="2"?"selected='selected'":""}>복지 문화에 대해 제안합니다.</option>
					<option value="3" ${categoryNum=="3"?"selected='selected'":""}>부당한 대우에 대해 신고합니다.</option>
					<option value="4" ${categoryNum=="4"?"selected='selected'":""}>근무환경에 대해 제안합니다.</option>
					<option value="5" ${categoryNum=="5"?"selected='selected'":""}>경영진에게 제안합니다.</option>
					<option value="6" ${categoryNum=="6"?"selected='selected'":""}>기타 의견을 제안합니다.</option>
				</select>
				<br><br>
				<input name="subject" type="text"  class="form-control" placeholder="제목을 입력해 주세요. ex) 회의 분위기가 너무 무거워요.">
				<br><br>
				<textarea name="content" id="content" class="form-control" cols="10" placeholder="내용을 입력해 주세요. ex) 단체 회의 분위기가 조금은 경직된 것 같아요. 다같이 좋은 아이디어를 낼 수 있도록 조금은 부드러운 분위기를 만들어 주세요." style="height: 150px;"></textarea>
				<br> 
