     ...
     <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide="next">
      ...
      <div class="card p-4 mb-4">
         ...
         <div>
            ...
            <button type="button" class="btn btn-sub" onclick="punchView()">출퇴근 현황</button>
         </div>
         </div>
...
function punchView() {
   $("form[name=searchForm] select[name=condition]").val("");
   $("form[name=searchForm] input[name=std]").val("");
   $("form[name=searchForm] input[name=end]").val("");
   listPage(1);
...
function listPage(page){
   ...
   if(f.condition.value == "" && f.std.value != ""){
      alert("구분과 끝 날짜를 선택하세요");
      return;
   } else if(f.condition.value == "" && f.end.value != "") {
      alert("구분과 시작 날짜를 선택하세요");
      return;
   } else if(f.condition.value == "" && f.std.value != "" && f.end.value != "") {
      alert("구분을 선택하세요");
      return;
   } 
   
   let query = $("form[name=searchForm]").serialize();
   let url = "${pageContext.request.contextPath}/listPunchclock";

   const fn = function(data){
      $(".punch-body").html(data);
   };
         ...
         <div class="card mt-4 p-4">
             ...
             <form name="searchForm">
                <div class="d-flex justify-content-center align-items-center pb-2 border-bottom ">
                    <select class="form-select me-2" name="condition" id="condition" style="width: 100px;">
                    ...
                    <button type="button" class="btn btn-main" onclick="listPage(1)"><i class="fa-solid fa-magnifying-glass"></i></button>
                </div>
             </form>
                <div class="m-auto punch-body"></div>
            </div>
          </div>  
      </div>
